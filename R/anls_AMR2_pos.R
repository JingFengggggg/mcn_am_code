...
s1.1 <- new_section2(
  ...
  rblock({
    mcn <- mcnebula()
    mcn <- initialize_mcnebula(mcn, "sirius.v5", "../sirius_AMR2_pos")
    ion_mode(mcn) <- "pos"
  }, F)
)
...
s3.1 <- new_section2(
  ...
  rblock({
    ...
    mcn <- cross_filter_stardust(mcn, min_number = 30, max_ratio = .09)
...
s3.5 <- new_section2(
  ...
  rblock({
    ...
    pattern <- c("fatty acid", "hydroxy","^Ar", "^Alk", "^Ben","^orga",
                 ...
                 "^Ox", "Ureas", "^Viny", "^Sul", "Piper","^Indo", "^Dia",
                 "Guanidines","^Dica","compounds$"
    )
...
s5.6 <- new_section2(
  ...
  rblock({
    ...
    ggsave(
      f5.61 <- paste0(tmp, "/parent_nebula.png"),
      p, width = 18, height = 15, dpi = 600
    )# export:width = 18, height = 15
    pdf(f5.62 <- paste0(tmp, "/child_nebula.pdf"), 14 * fig.s, 15 * fig.s)
    visualize_all(mcn)
    dev.off()
  })
)
...
s8.2 <- new_section2(
  ...
  rblock({
    pdf(f8.2 <- paste0(tmp, "/tracer_child_nebula.pdf"), 30 * 1.2,  35* 1.2)
    visualize_all(mcn2)
    dev.off()
  })
)
...
s9.1 <- new_section2(
  ...
  rblock({
    ...
    pdf(f9.1 <- paste0(tmp, "/logFC_child_nebula.pdf"), 30, 40)
...
s10.2 <- new_section2(
  ...
  rblock({
    ...
    focus_classes <- c("Sesquiterpenoids")
...
s10.3 <- new_section2(
  ...
  rblock({
    ...
    for (i in focus_classes) {
      ...
      ggsave(fs[[ i ]] <- paste0(tmp, "/", gsub(" ", "_", i), "_child.pdf"), 
             p, width = 5, height = 100, limitsize = FALSE
      )# export:width = 100, height = 100
    }
    ses <- c("759", "1301", "1417", "1813", "2580", "5543", "7337", "9568")
    for (i in ses) {
      pdf(paste0(tmp, "/feature_", i, ".pdf"), 18, 6)
      show_node(mcn2, i)
      dev.off()
    }
  })
...
s10.3.fig1 <- include_figure(fs[[1]], "Sesquiterpenoids", paste0("Annotated Nebulae: ", names(fs[1])))

# ms/ms-spectra -----------------------------------------------------------


s12 <- new_heading("Plot MS/MS spectra of top ‘features’ in Sesquiterpenoids", 2)

s12.1 <- new_section2(c("Drawing of MS/MS spectra of top ‘features’ in Sesquiterpenoids."),
                     rblock({
                       mcn2 <- draw_structures(mcn2, .features_id = ses)
                       pdf(f12.1 <- paste0(tmp, "/msms_ses_identified.pdf"), 12, 12)
                       plot_msms_mirrors(mcn2, ses)
                       dev.off()
                     }))

s12.1.fig1 <- include_figure(f12.1, "msms","MS/MS spectra of top ‘features’ in Sesquiterpenoids (identified)")

s12.2 <- c("See results",ref(s12.1.fig1),".")

s12.3 <- new_section2(c("Plot EIC spectra of top ‘features’ in Sesquierpenoids. (Since the code below requires .mzML mass spectrometry data, these files
are too large to be stored in a package, so the code below will not be run. But we have saved the result
data.)"),
                    rblock({
                      metadata$file <- paste0(metadata$sample, ".mzML")
                      data <- plot_EIC_stack(ses, metadata,
                                             quant.path = paste0(
                                               tmp,
                                               "/../pos_msms.csv"
                                             ), 
                                             mzml.path = paste0(
                                               tmp, "/../../../.mzML/AMR2_pos.mzml"
                                             ),
                                             palette = palette_stat(mcn2)
                      )
                      save(data, file = paste0(tmp, "/eic_ses_data.rdata"))
                    }))

s12.4<-new_section2(c("Load the saved data and draw the figure."),
                    rblock({
                      load(paste0(tmp, "/eic_ses_data.rdata"))
                      pdf(f12.4 <- paste0(tmp, "/eic_ses_identified.pdf"), 12, 12)
                      print(data$p)
                      dev.off()
                    }))

s12.5<-new_section2(c("Or use following to re-plot the ‘ggplot’ object."),
                    rblock({
                      data <- plot_EIC_stack(data = data, palette = palette_stat(mcn2))
                    }))

s12.4.fig1 <- include_figure(
  f12.4, "/eic", 
  "Extracted Ions Chromatograph (EIC) of top ‘features’ in Sesquiterpenoids (identified)"
)

s12.5<-c("See results", ref(s12.4.fig1))

# export-cytoscape --------------------------------------------------------


s14 <- new_heading("Export for Cytoscape", 2)
  ...
  rblock({
    ...
    lapply(
      ...
      function(name) {
        igraph::write_graph(
          res[[name]],
          file = paste0(tmp, "/graphml", "/", name, ".graphml"),
          format = "graphml"
        )
      })
  })
...
render_report(report, file <- paste0(getwd(), "/report.Rmd"))
